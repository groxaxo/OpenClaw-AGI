...
#
# The trainer script starts sglang on GPU 1 automatically.
# The OpenClaw proxy (FastAPI) runs inside the trainer on port $PROXY_PORT.
...
# ── GPU assignment ─────────────────────────────────────────────────────────
TRAINING_GPU="${TRAINING_GPU:-0}"   # GPU index for QLoRA training
ROLLOUT_GPU="${ROLLOUT_GPU:-1}"     # GPU index for sglang inference
...
# ── sglang rollout server (launched by the trainer, runs on ROLLOUT_GPU) ───
...
# ── PRM (optional; disable to save memory) ────────────────────────────────
PRM_ENABLE="${PRM_ENABLE:-0}"   # set to 1 to enable
...
# Build the PRM flag only when enabled
PRM_ARGS=()
...
# ── launch the trainer on TRAINING_GPU ────────────────────────────────────
# sglang is launched inside the trainer and automatically uses ROLLOUT_GPU
# (set via CUDA_VISIBLE_DEVICES ordering; the trainer script receives
# only GPU 0 visible, so we must pass both devices and let sglang pick
# the second one via --sglang-tp and environment ordering).

# Export device visibility: training script sees GPU 0, sglang subprocess
# is spawned with its own CUDA_VISIBLE_DEVICES pointing to ROLLOUT_GPU.
export CUDA_VISIBLE_DEVICES="${TRAINING_GPU}"

# Tell sglang which physical GPU to use by passing the real device ID via
# the environment variable that the trainer forwards to the subprocess.
export SGLANG_CUDA_VISIBLE_DEVICES="${ROLLOUT_GPU}"

PYTHONPATH="${SCRIPT_DIR}:${PYTHONPATH:-}" \
CUDA_VISIBLE_DEVICES="${TRAINING_GPU}" \
python "${SCRIPT_DIR}/unsloth_qlora_trainer.py" \
